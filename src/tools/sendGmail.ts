...
function buildRawMime(params: { to: string; subject: string; body: string }): string {
  const from = GMAIL_SENDER_NAME ? `${GMAIL_SENDER_NAME} <${GMAIL_SENDER}>` : GMAIL_SENDER;
...
export async function sendGmail(params: {
  ...
  const preview = `Subject: ${params.subject}\nTo: ${params.to}\n\n${params.body}`;
