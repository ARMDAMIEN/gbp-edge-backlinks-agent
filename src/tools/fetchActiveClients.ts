...
export async function fetchActiveClients(): Promise<{ clients: ActiveClient[]; skipped: number }> {
  ...
  for (const page of results) {
    ...
    if (!status.includes("active") && !status.includes("actif")) continue;

    const nameProp =
      pickProp(props, ["Name", "Nom", "Business", "Business Name", "Client"]) ??
    ...
    const niche = plainText(
      pickProp(props, ["Type d'activité", "Niche", "Industry", "Industrie", "Secteur", "Category", "Catégorie", "Activité"])
    );
    const email = plainText(pickProp(props, ["Email", "Contact Email", "Mail"]));
    const place_id = plainText(
      pickProp(props, ["LocationId", "Location Id", "Place ID", "Place Id", "PlaceId", "GBP Place Id"])
    );
